...
annual_financials = function(db_pw){
  ...
  nn = dbSendQuery(con, "select * from farm_db.yearly_agg_financials order by year_key")
  nn = dbFetch(nn)
  #dbClearResult(nn)
  print(head(nn))
  nn[is.na(nn)] = 0
...
annual_financials_agg = function(db_pw){
  ...
  nn = dbSendQuery(con, "select * from farm_db.yearly_agg_agg_financials order by year_key")
  nn = dbFetch(nn)
  #dbClearResult(nn)
  print(head(nn))
  nn[is.na(nn)] = 0
